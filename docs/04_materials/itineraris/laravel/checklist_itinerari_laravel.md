...
La revisió final de l'itinerari no s'ha de limitar al fet que l'aplicació arranque. També ha de comprovar autenticació, persistència, validació, proves, documentació, `AI log` i capacitat real de continuar el projecte base sobre `Laravel`.
...
- també ha de deixar identificades les peces pròpies de `Laravel` que funcionaran com a patró reusable i comparable amb els altres itineraris

## Errors habituals o riscos

- confondre "projecte creat" amb "itinerari preparat"
- tindre arrancada tècnica però no recorregut clar cap als Reptes 2-5
