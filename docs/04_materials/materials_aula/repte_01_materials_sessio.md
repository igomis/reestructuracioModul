...
- Sessió `1` -> `R1M1`: model client/servidor i decisió tècnica inicial orientativa.
- Sessió `2` -> `R1M2`: entorn executable, landing inicial servida pel backend, documentació i checkpoint.

## Mini-briefings de les sessions clau

### Sessió 1. R1M1. Model client/servidor i decisió tècnica inicial
...
### Sessió 2. R1M2. Entorn executable, landing inicial i tancament

- Obertura possible: recuperació de la decisió tècnica de `R1M1`.
- Objectiu de la sessió: acabar l'entorn, crear una landing inicial servida pel backend i deixar documentació útil dins del repositori.
- Què s'espera al final: projecte arrancant, serveis mínims definits, landing inicial del producte, `README`, documentació en repositori i una decisió o incidència registrada.
- Error habitual a evitar: copiar la demo del professorat sense adaptar-la ni saber explicar què fa cada servei.
- Evidència mínima del dia: arrencada reproduïble, BBDD i phpMyAdmin incorporats si cal, landing inicial servida pel backend i documentació enllaçada.
...
- `CP-R1.2`
  - Què hauria d'estar fet: entorn executable, landing inicial servida pel backend, `README`, documentació i checkpoint de pas a `R2`.
  - Com es verifica en `2-3` minuts: arrancar projecte, obrir la landing, llegir els passos des del `README` i explicar què s'ha adaptat respecte del model docent.
...
- Vas bé: `Ja tens clar el paper del servidor i l'entorn arranca. Ara deixa la landing inicial servida pel backend i documenta-la bé.`
...
- Tens marge per ampliar: `Si ja tens el mínim de R1, pots millorar el README, l'onboarding o la claredat de la landing, però sense obrir encara el flux complet de R2.`

## Fulls de treball base contextualitzats

### Full 1. Model client/servidor i decisió tècnica
...
- Objectiu: deixar el projecte arrancant, amb una landing inicial servida pel backend i documentació suficient.
- Tasca: partir del model mínim mostrat pel professorat, adaptar-lo al projecte propi, completar l'entorn, crear una landing amb HTML/CSS senzill i documentar passos.
- Evidència a generar: `README` inicial, prova d'arrencada reproduïble, landing inicial, decisió tècnica curta i incidència o dubte registrat.
- Validació mínima: una altra persona pot seguir els passos essencials, obrir la landing servida pel backend i entendre què fa cada servei.
...
- reduir el repte a una única decisió tècnica clara i una landing molt simple del producte
- evitar obrir formularis, fluxos complets o persistència si encara no arranca el projecte
- revisar primer `README`, arrencada i landing abans de tocar disseny o estructura extra
...
- afegir una pàgina `Sobre el projecte` o una segona resposta simple del backend sense obrir encara el flux complet de dades
...
- Sessió `2`: projecte arrancant, BBDD i phpMyAdmin incorporats si cal, landing inicial servida pel backend, `README`, documentació en repositori i decisió/incidència registrada.

## Connexió amb el repte següent

El `R1` només queda ben preparat si deixa una base executable i una landing inicial real del backend que puga créixer en `R2`. Si al final d'este repte només hi ha infraestructura o només hi ha intenció, el pas a sessions i auth començarà sobre una base falsa.
