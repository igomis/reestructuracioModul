...
- les adaptacions de materials s’han de traduir en artefactes professionals: README, nota tècnica de decisió, checklist, proves, documentació API, registre de debugging
...
Clau metodològica del mapa:

- `R2` es treballa sobre base comuna en `PHP`
- els materials d'itinerari entren a partir de `R3`
- les vies base del contrast són `Laravel`, `Symfony` i `NestJS`
- `FastAPI` només s'ha de llegir com a via avançada o excepcional
...
|---|---|---|---|---|---|---|
| Bloc 1. Arquitectures web | Repte 1 (principal), Repte 3 (reforç) | RA1, inici de RA5 | Justificar stack, entorn i decisions de kickoff; recuperar criteri arquitectònic en la refactorització MVC. | Reduir teoria enciclopèdica i convertir-la en guia de decisió amb nota tècnica de decisió i criteris de trade-off. | Comparar alternatives, generar esborrany d’nota tècnica de decisió i qüestionari de riscos tècnics. | Material d’entrada; una part passa de “tema” a “suport de decisió” reutilitzable en R1 i R3. |
...
| Blocs 3 i 4. Framework i dades | Repte 3 (principal), Repte 4 (reforç), Repte 5 (consolidació) | RA5, RA6 (reforç RA8 i RA7) | Refactoritzar a MVC o equivalent, consolidar persistència segura i deixar preparada la capa de servei per a API. | Tractar 3 i 4 com una base de contrast: arquitectura per capes, modelat de dades, validació i proves d’integració. | Donar pistes de refactorització, suggerir casos de prova i revisar coherència entre models/controladors, sense implementar-los. | Nucli tècnic del mòdul; cal eliminar duplicació entre teoria MVC i accés a dades. |
...
- de “tema” a “suport tècnic”: comparatives d’arquitectura, plantilla d’nota tècnica de decisió i checklist d’arrancada
- materials a fusionar/retallar: introduccions llargues repetides entre apunts d’arquitectura i orientacions de kickoff
- evidències d’ús: nota tècnica de decisió, README executable, defensa tècnica curta de la decisió
...
- redundància entre introduccions generals de Bloc 1 i documentació de kickoff (README i nota tècnica de decisió): convé deixar una sola peça de decisió tècnica
...
| Introducció a decisions arquitectòniques | Bloc 1 | Repte 1 (kickoff) + reforç Repte 3 | Reduir teoria llarga i convertir-la en plantilla d’nota tècnica de decisió + checklist de decisió. |
