...
La duració no ha de ser sempre la mateixa. Si el taller només introdueix criteri o dona una pauta de revisió, pot ocupar `15-30` minuts. Si exigeix posar en marxa codi, proves, configuració o una decisió tècnica amb evidència al repositori, ha d'ocupar `35-60` minuts. Les variants opcionals només s'activen si aporten valor al projecte real.
...
| `MT02` | `R1S2`, `2:05-2:35` | README executable i commit defensable | `30 min` | revisió guiada de documentació i traçabilitat sobre treball ja fet | [Guia README i ADR](../04_materials/materials_comuns/guia_readme_i_adr.md) i [Guia Git](../04_materials/materials_comuns/guia_git_i_flux_treball.md) | [MT02](../07_presentacions/microtallers/mt02_readme_commit_defensable.md) | instruccions d'arrancada i commit explicable |
...
| `MT16` | `R3S2B` o, si no hi ha sessió extra, `R3S2`, `2:10-3:00` | Portabilitat entre frameworks | `50 min` opcional | és auditoria comparativa; només ocupa sessió completa si el centre vol taller ampliat | [Model d'itineraris tecnològics](model_itineraris_tecnologics.md) i itineraris | [MT16](../07_presentacions/microtallers/mt16_portabilitat_frameworks.md) | taula d'equivalències i decisió de portabilitat |
...
|---|---|---|---|
| `MT16` | després de `R3S2`, abans de `R3S3` | si hi ha comparació real entre Laravel, Symfony i NestJS | `50 min` dins de `R3S2` o sessió ampliada `R3S2B` si el centre té marge |
