...
- Integració triada perquè la IA la proposa, però sense valor per al domini.
- Transformació de dades no validada amb resposta real.
- Workflow o connector que només funciona amb un exemple preparat.
- Documentació final elegant però no reproduïble.

## Mesures de control i verificació

- Provar una resposta real o una mostra guardada amb estructura coneguda.
- Demanar explicació del mapping o transformació camp a camp.
- Revisar què queda en el producte quan el servei extern falla.
- Contrastar `README`, commits, proves i `AI log`.
