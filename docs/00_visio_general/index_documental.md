# Index documental del projecte

## Finalitat
Este document organitza l'acces al repositori per camins d'us i no com a llista plana. La idea no es repetir tot el contingut, sino donar una porta d'entrada curta segons la necessitat real de qui consulta el projecte.

## Si vull entendre el projecte en 10 minuts
- [Presentacio breu del model DWES](presentacio_model_dwes_reptes_ia.md): versio curta del model per reptes amb IA.
- [Resum executiu del paquet docent](resum_executiu_paquet_docent.md): vista general del paquet ja consolidat.
- [Mapa modul -> reptes](mapa_modul_reptes.md): relacio entre el modul i la sequencia de reptes.
- [Enunciat base del projecte del curs](../05_projectes_tecnics/enunciat_projecte_base.md): producte central sobre el qual es desplega el curs.

## Si vull preparar el curs
- [Index de programacio del modul](../01_programacio_modul/index.md): porta d'entrada al bloc de planificacio docent.
- [Curs executable DWES en 2 avaluacions](../01_programacio_modul/curs_executable_dwes_2_avaluacions.md): lectura unificada del curs.
- [Guia sessio a sessio del curs en 20 setmanes](../01_programacio_modul/guia_sessio_a_sessio_20_setmanes.md): sequencia fina per setmanes.
- [Preparacio docent directa per al curs que ve](../01_programacio_modul/preparacio_docent_directa_curs_que_ve.md): que ha d'arribar preparat abans de setembre.
- [Ajustos finals abans de l'inici de curs](../01_programacio_modul/ajustos_finals_abans_inici_curs.md): ultima revisio operativa abans de l'arrancada.

## Si vull consultar l'avaluacio
- [Index del bloc d'avaluacio](../03_avaluacio/index.md): porta d'entrada a rubriques, evidencies i seguiment.
- [Sistema d'evidencies](../03_avaluacio/sistema_evidencies.md): base de la lectura i verificacio del treball.
- [Rubrica base dels reptes](../03_avaluacio/rubrica_base_reptes.md): criteri d'avaluacio del producte al llarg del curs.
- [Rubrica de defensa tecnica](../03_avaluacio/rubrica_defensa_tecnica.md): criteri de defensa individual.
- [Instruments de seguiment docent](../03_avaluacio/instruments_seguiment_docent.md): seguiment operatiu del curs.

## Si vull nomes materials d'aula
- [Index del bloc de materials](../04_materials/index.md): entrada curta a materials comuns, materials d'aula i apunts reals.
- [Mini briefings setmanals](../04_materials/materials_aula/mini_briefings_setmanals.md): activacio rapida de sessions.
- [Checkpoints curts d'aula](../04_materials/materials_aula/checkpoints_curts_aula.md): control curt de progres.
- [Fulls de treball base](../04_materials/materials_aula/fulls_treball_base.md): suport d'operativa a classe.
- [Revisio transversal dels apunts reals](../04_materials/apunts_reals/revisio_transversal_apunts_reals.md): entrada a la serie d'apunts ja consolidats.

## Si vull preparar el pilotatge
- [Kit de pilotatge de les setmanes 1-4](../01_programacio_modul/kit_pilotatge_setmanes_01_04.md): arrancada del curs.
- [Kit de pilotatge de les setmanes 5-8](../01_programacio_modul/kit_pilotatge_setmanes_05_08.md): desplegament de `R2`.
- [Kit de pilotatge de les setmanes 9-12](../01_programacio_modul/kit_pilotatge_setmanes_09_12.md): entrada a `R3` i tancament de la 1a avaluacio.
- [Kit de pilotatge de les setmanes 13-16](../01_programacio_modul/kit_pilotatge_setmanes_13_16.md): consolidacio de `R3` i obertura de `R4`.
- [Kit de pilotatge de les setmanes 17-20](../01_programacio_modul/kit_pilotatge_setmanes_17_20.md): `R5`, defensa final i tancament.
- [Revisio transversal del bloc pilotable de 20 setmanes](../01_programacio_modul/revisio_transversal_bloc_pilotable_20_setmanes.md): verificacio comuna del bloc `1-20`.
- [Pla de pilotatge real](pla_pilotatge_real.md), [pla d'execucio](pla_execucio_pilotatge_real.md) i [seguiment](seguiment_execucio_pilotatge.md): capa de pilotatge i feedback.

## Si vull adaptar el model a un altre modul
- [Paquet docent exportable del modul](paquet_docent_exportable.md): quines peces son transferibles.
- [Versio prepilotatge v1](versio_prepilotatge_v1.md): que es considera estable i reutilitzable.
- [Programacio d'aula per repte](../01_programacio_modul/programacio_aula_per_repte.md): lectura docent reutilitzable per blocs.
- [Model d'itineraris tecnologiques](../01_programacio_modul/model_itineraris_tecnologics.md): criteri per comparar stacks sense canviar el model.
- [Criteris de ponderacio dels reptes amb IA](../03_avaluacio/criteris_ponderacio_reptes_amb_ia.md): base d'avaluacio adaptable.

## Acces per blocs
- [Visio general](../00_visio_general/index_documental.md)
- [Programacio del modul](../01_programacio_modul/index.md)
- [Avaluacio](../03_avaluacio/index.md)
- [Materials](../04_materials/index.md)
