...
- **RA7**: quan la integració reutilitza o orquestra l'API pròpia com a part del flux híbrid
...
Per això, curricularment s'ha de llegir com el lloc natural de desplegament de **RA9**. No és un afegit ornamental al final del curs: és el repte que comprova si l'alumnat sap integrar serveis, fonts externes o repositoris heterogenis dins d'una aplicació híbrida amb valor real. `RA7` i `RA8` poden reforçar el repte, però no n'han de desplaçar el centre.
...
- CA de **RA9** vinculats a selecció de fonts externes, consum real, transformació, integració híbrida, prova i documentació del flux
- CA de suport de **RA7** quan la integració reutilitza o orquestra serveis propis
...
- mapa breu del flux híbrid o de la integració entre components
...
- L’alumnat identifica tecnologies, frameworks o fonts aplicables a una integració híbrida real.
